import {
    LENGTH_COORDINATION_KEY,
    TYPE_COORDINATION_FORWARD_IPV6_WEBSOCKET
} from '../constants/index.mjs';

import ElGamalCryptography from '../cryptography/elgamal/index.mjs';
import TwofishCryptography from '../cryptography/twofish/index.mjs';
import CoordinationPackets from '../packets/coordination/format/index.mjs';
import RandomGenerator from '../random/index.mjs';
import AnonymityRouter from './index.mjs';

import { expect } from 'chai';
import { WebSocketServer, WebSocket } from 'ws';

const LENGTH_HEADER = 256; // octets, also known as "bytes"
const LENGTH_KEY = 256; // octets, also known as "bytes"

const OFFSET_KEY_SENDER = LENGTH_HEADER;
const OFFSET_DATA_START = LENGTH_HEADER + LENGTH_KEY;

const TIMEOUT_TEST = 15000; // milliseconds
const TIMEOUT_NETWORK = 12000; // milliseconds

const PORT_ANLA_DEFAULT = 11412; // 'A' = 1, 'N' = 14, 'L' = 12

const HOST_ANY = '::';
const PORT_ANY = 0;

const KEY_DECRYPTION = Uint8Array.from([
    0x00, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07,
    0x08, 0x09, 0x0A, 0x0B, 0x0C, 0x0D, 0x0E, 0x0F,
]);

const BLOB_HELLO_WORLD = Uint8Array.from([
    0x48, 0x65, 0x6C, 0x6C, 0x6F, 0x2C, 0x20, 0x57,
    0x6F, 0x72, 0x6C, 0x64, 0x21, 0x0A, 0x00, 0x00,
]);

const PACKET_LENGTH = LENGTH_HEADER + LENGTH_KEY + BLOB_HELLO_WORLD.length;

const BLOB_PRIVATE_KEY = Uint8Array.from([
    ,
]);

const BLOB_SENDER_KEY = Uint8Array.from([
    ,
]);

const REMOTE_HOST = '::1';
const REMOTE_PORT = PORT_ANLA_DEFAULT;

describe('AnonymityRouter', () => {

    it('should forward coordination packets using IPv6 and WebSockets', (done) => {

        const testData = Object.seal({
            messagesMatch: false,
            testComplete: false,
            count: 0,
        });

        const announceError = (error) => {

            if (!testData.testComplete) {

                done(error);
                testData.testComplete = true;

            }

        };

        const optionsRemoteServer = {
            exponent: BLOB_PRIVATE_KEY,
            coordination: {
                type: 'websocket',
                host: REMOTE_HOST,
                port: REMOTE_PORT,
            },
            transport: {
                type: 'websocket',
                host: REMOTE_HOST,
                port: PORT_ANY,
            },
        };

        const remoteServer = AnonymityRouter.createServer(optionsRemoteServer);

        const remoteConnection = (remoteHost, remotePort, packet) => {

            const target = `ws://[${ remoteHost }]:${ remotePort }/`;

            const webSocket = new WebSocket(target, {
                perMessageDeflate: false
            });

            webSocket.on('error', (error) => {

                announceError(error);

            });

            webSocket.on('open', () => {

                webSocket.send(packet);

                setTimeout(() => {

                    webSocket.close();

                }, 1000);

            });

        };

        const startLocalServer = (remoteHost, remotePort) => {

            const options = {
                host: HOST_ANY,
                port: PORT_ANY,
            };

            const localServer = new WebSocketServer(options);
            const shutdownServers = () => {

                if (!testData.testComplete) {

                    const error = new Error('timeout without reply!');
                    announceError(error);

                }

                remoteServer.close();
                localServer.close();

            };

            localServer.on('error', announceError);

            localServer.on('close', () => {

                try {

                    expect(testData.messagesMatch).to.equal(true);

                } catch (error) {

                    announceError(error);

                }

            });

            localServer.on('listening', () => {

                const address = localServer.address();
                const { address: host, port } = address;

                const text = {
                    type: TYPE_COORDINATION_FORWARD_IPV6_WEBSOCKET,
                    key: KEY_DECRYPTION,
                    lengthReal: BLOB_HELLO_WORLD.length,
                    lengthNext: BLOB_HELLO_WORLD.length,
                    destination: { host, port },
                };

                const packet = new Uint8Array(PACKET_LENGTH);
                CoordinationPackets.format(text, packet);

                const keyPublic = new Uint8Array(LENGTH_KEY);
                ElGamalCryptography.calculateKey2048(
                    BLOB_PRIVATE_KEY,
                    keyPublic,
                );

                const encryptedHeader = new Uint8Array(LENGTH_HEADER);
                const keySender = packet.subarray(OFFSET_KEY_SENDER);

                ElGamalCryptography.encrypt2048(
                    keyPublic,
                    BLOB_SENDER_KEY,
                    packet,
                    encryptedHeader,
                    keySender,
                );

                packet.set(encryptedHeader);

                const key = KEY_DECRYPTION;
                const textPlain = BLOB_HELLO_WORLD;
                const textCipher = packet.subarray(OFFSET_DATA_START);
                TwofishCryptography.encrypt128(key, textPlain, textCipher);

                remoteConnection(remoteHost, remotePort, packet);

            });

            localServer.on('connection', (socket) => {

                socket.on('error', announceError);

                socket.on('message', (message) => {

                    try {

                        const binaryA = BLOB_HELLO_WORLD;
                        const binaryB = message;

                        let index = 0;
                        const lengthA = binaryA.length;
                        const lengthB = binaryB.length;
                        while (index < lengthA && index < lengthB) {

                            expect(binaryA[index]).to.equal(binaryB[index]);

                            index = (index + 1) | 0;

                        }

                        testData.messagesMatch = true;
                        testData.testComplete = true;
                        done();

                    } catch (error) {

                        announceError(error);

                    }

                });

            });

            setTimeout(shutdownServers, TIMEOUT_NETWORK);

        };

        startLocalServer(REMOTE_HOST, REMOTE_PORT);

    }).timeout(TIMEOUT_TEST);

    ////////////////////////////////////////////////////////////////////
    // The routers should strive towards a "everyone-sees-everyone"   //
    // kind of situation, as this prevents some few nodes growing in  //
    // excessive relevance and ruining the decentralization of the    //
    // network. However, this is not a behavior that is required of   //
    // all implementations (and the protocol accommodates for this).  //
    // Some routers may be more silent and only advertise themselves  //
    // for a select few. The following test dot not reflect all       //
    // possible implementations of this protocol.                     //
    ////////////////////////////////////////////////////////////////////

    it('should eventually become a complete, fully-connected graph', (done) => {

        // four routers are created and initially linked like this:
        // 1 -> 2 -> 3 -> 4 -> 1
        // This is not a complete, fully-connected graph. However,
        // as the nodes advertise themselves to each other, it should
        // become a "everyone-sees-everyone" graph, a complete graph.

        const LENGTH_REMOTE_SERVERS = 4;
        const MASK_MODULUS_FOUR = 0b11;

        const testData = Object.seal({
            serversListening: 0,
            completeGraph: false,
            testComplete: false,
        });

        const announceError = (error) => {

            if (!testData.testComplete) {

                done(error);
                testData.testComplete = true;

            }

        };

        RandomGenerator.seed(performance.now() | 0);
        const exponent = new Uint8Array(LENGTH_COORDINATION_KEY);

        const optionsRemoteServer = {
            exponent: exponent,
            coordination: {
                type: 'websocket',
                host: REMOTE_HOST,
                port: PORT_ANY,
            },
            transport: {
                type: 'websocket',
                host: REMOTE_HOST,
                port: PORT_ANY,
            },
        };

        const remoteServers = new Array(LENGTH_REMOTE_SERVERS);

        const linkServers = () => {

            let indexA = 0;
            while (indexA < LENGTH_REMOTE_SERVERS) {

                const indexB = (indexA + 1) & MASK_MODULUS_FOUR;

                const remoteServerA = remoteServers[indexA];
                const remoteServerB = remoteServers[indexB];

                const peerData = { ...remoteServerA.getPeerData(), self: false };
                remoteServerB.addPeer(peerData);

                indexA = (indexA + 1) | 0;

            }

        };

        let index = 0;
        while (index < LENGTH_REMOTE_SERVERS) {

            RandomGenerator.fill(exponent);

            const remoteServer = AnonymityRouter.createServer(optionsRemoteServer);
            remoteServers[index] = remoteServer;

            remoteServer.on('listening', () => {

                testData.serversListening = (testData.serversListening + 1) | 0;
                if (testData.serversListening === 4) {

                    linkServers();

                }

            });

            remoteServer.on('error', announceError);

            index = (index + 1) | 0;

        }

        let cleanupCallback;

        const checkCompleteGraph = () => {

            let index = 0;
            while (index < LENGTH_REMOTE_SERVERS) {

                const { peers } = remoteServers[index];
                if (peers.size < 4) {

                    testData.completeGraph = false;
                    return;

                }

                index = (index + 1) | 0;

            }

            testData.completeGraph = true;
            testData.testComplete = true;
            done();

            if (typeof cleanupCallback === 'function') {

                cleanupCallback();

            }

        };

        const intervalCheck = setInterval(checkCompleteGraph, 1000);

        const shutdownServers = () => {

            clearInterval(intervalCheck);

            let index = 0;
            while (index < LENGTH_REMOTE_SERVERS) {

                remoteServers[index].close();

                index = (index + 1) | 0;

            }

            if (testData.testComplete) {

                return;

            }

            const error = new Error('timeout without complete graph!');
            announceError(error);
            return;

        };

        setTimeout(shutdownServers, TIMEOUT_NETWORK);
        cleanupCallback = shutdownServers;

    }).timeout(TIMEOUT_TEST);

});

