...
const setup = () => {
    ...
    while (index < 4) {
        ...
        wordsA[index] = (Math.random() * (1 << 31)) >>> 0;
        wordsB[index] = (Math.random() * (1 << 31)) >>> 0;
        wordsC[index] = (Math.random() * (1 << 31)) >>> 0;
...
const generateOctet = () => {
    ...
    const randomWord = ((wordC + (wordB | 1)) | 0) ^ wordA;
    wordsA[indexA] = randomWord ^ (~wordB);
    wordsB[indexB] = randomWord ^ (~wordC);
    wordsC[indexC] = randomWord ^ (~wordA);

    const randomOctet = POLYNOMIAL_MODULAR_REMAINDER(randomWord, polynomial);

    indexA = (indexA + (randomOctet >> 1)) & MASK_MODULUS_FOUR;
    indexB = (indexB + (randomOctet >> 3)) & MASK_MODULUS_FOUR;
    indexC = (indexC + (randomOctet >> 5)) & MASK_MODULUS_FOUR;

    chosenPolynomial = (chosenPolynomial ^ randomOctet) & MASK_MODULUS_EIGHT;
